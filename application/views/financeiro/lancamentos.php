            ...
            <div class="widget-content nopadding tab-content">
                ...
                <table class="table table-bordered " id="divLancamentos">
                    <thead>
                        <tr>
                            ...
                            <th>Status</th>
                            <th>Observações - Detalhes como foi Pago...</th>
              ...
              <td colspan="10" >Nenhum lançamento encontrado</td>
                        ...
                        foreach ($results as $r) {
                            ...
                            echo $r->teste != 0 ? '<td>' . $r->teste . '%</td>' : '<td> </td>'; //nao funciona esta apenas funcionando como espaço!
                            //echo $r->desconto != 0 ? '<td>R$ ' . number_format($r->valor_desconto/(1-($r->desconto/100)), 2, ',', '.') . '</td>' : '<td> </td>';
                          //  echo '<td> R$ ' . number_format($total, 2, ',', '.') . '</td>'; $r->valor_desconto/(1-($r->desconto/100)) number_format($r->valor_desconto/(1-($r->desconto/100)), 2, ',', '.')
                            echo '<td>';
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eLancamento')) {
                                echo '<a href="#modalEditar" style="margin-right: 1%" data-toggle="modal" role="button" idLancamento="' . $r->idLancamentos . '" descricao="' . $r->descricao . '" valor="' . $r->valor . '" vencimento="' . date('d/m/Y', strtotime($r->data_vencimento)) . '" pagamento="' . date('d/m/Y', strtotime($r->data_pagamento)) . '" baixado="' . $r->baixado . '" cliente="' . $r->cliente_fornecedor . '" tipo="' . $r->tipo . '" observacoes="' . $r->observacoes . '" usuario="' . $r->nome . '" class="btn-nwe3 editar" title="Editar OS"><i class="bx bx-edit"></i></a>';
                            }
    ...
    <form id="formReceita" action="<?php echo base_url() ?>index.php/financeiro/adicionarReceita" method="post">
        ...
        <div class="modal-body">
            ...
            <div class="span12" style="margin-left: 0">
                ...
                <div class="span12" style="margin-left: 0">
                    <label for="observacoes">Observações - Ex. Detalhar como foi Pago...</label>
