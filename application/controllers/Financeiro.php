    ...
    public function editar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->financeiro_model->edit('lancamentos', $data, 'idLancamentos', $this->input->post('id')) == true) {
                if($data['baixado'] == 1){
                    $this->db->where('idLancamentos', $this->input->post('id'));
                    $this->db->limit(1);
                    $lanc = $this->db->get('lancamentos')->row();
                    $this->load->model('os_model');
                    $status = ['status' => 'Entregue - Faturado',];
                    $this->os_model->edit('os', $status, 'idOs', $lanc->os_id);
                }
