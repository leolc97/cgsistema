...
class Mapos extends MY_Controller
{
    ...
    {
        $this->data['andamento'] = $this->mapos_model->getOsIndex("Em Andamento");
        $this->data['aguardandotec'] = $this->mapos_model->getOsIndex("Aguardando Análise Técnica");
        $this->data['aguardandocl'] = $this->mapos_model->getOsIndex("Aguardando aprovação do cliente");
        $this->data['aguardandop'] = $this->mapos_model->getOsIndex("Aguardando Peças");
        $this->data['prontoag'] = $this->mapos_model->getOsIndex("Pronto - Aguardando Retirada");
        $this->data['aguardandoret'] = $this->mapos_model->getOsIndex("Aguardando Retirada - Cliente já Informado(a)");
        $this->data['entreguerec'] = $this->mapos_model->getOsIndex("Entregue - A Receber");
        $this->data['garan'] = $this->mapos_model->getOsIndex("Garantia");
    ...
    public function do_upload_logos($folder)
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmitente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar emitente.');
            redirect(base_url());
        ...
        $image_upload_folder = FCPATH . 'assets/uploads/' . $folder . '/';
    ...
    public function editarLogoEmitente()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmitente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar emitente.');
            redirect(base_url());
        ...
        $this->load->helper('file');
        delete_files(FCPATH . 'assets/uploads/emitente/');

        $image = $this->do_upload_logos('emitente');
        $logo = base_url() . 'assets/uploads/emitente/' . $image;
    ...
    public function editarLogos()
    {
        ...
        $this->load->helper('file');
        delete_files(FCPATH . 'assets/uploads/logo/');

        $image = $this->do_upload_logos("logo");
        $logo = base_url() . 'assets/uploads/logo/' . $image;

        $retorno = $this->mapos_model->editLogos($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
            log_info('Alterou a logomarca do emitente.');
        } else {
    ...
    public function editarLogosIcon()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmitente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar emitente.');
            redirect(base_url());
        ...
        $this->load->helper('file');
        delete_files(FCPATH . 'assets/uploads/favicon/');

        $image = $this->do_upload_logos("favicon");
        $logo = base_url() . 'assets/uploads/favicon/' . $image;

        $retorno = $this->mapos_model->editLogosIcon($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
            log_info('Alterou a logomarca do emitente.');
        } else {
    ...
    public function configurar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'termo_venda' => $this->input->post('termo_venda'),
                'termo_uso_Termica' => $this->input->post('termo_uso_Termica'),
                'termo_venda_Termica' => $this->input->post('termo_venda_Termica'),
        ...
        $this->data['dados'] = $this->mapos_model->getLogos();
    ...
    public function calendario()
    {
        ...
        $events = array_map(function ($os) {
            ...
            $totalServico = $totais['totalServico'];
            ...
            return [
                ...
                'extendedProps' => [
                    ...
                    'total' => '<b>Valor Total:</b> R$ ' . number_format($totalServico + $totalProdutos - $os->descontoproduto - $os->descontoservico, 2, ',', '.'),
