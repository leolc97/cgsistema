...
class Os_model extends CI_Model
{
    ...
    public function getProdutos($id = null) // select all products by id os
    {
    ...
    public function getProdutosId($id = null) // select a only product by your idProdutos_os
    {
        $this->db->get('*');
        $this->db->from('produtos_os');
    ...
    public function getServicosId($id = null)
    {
        $this->db->get('*');
        $this->db->from('servicos_os');
    ...
    public function valorTotalOS($id = null)
    {
        ...
        if ($valorDescontoBD = $this->getById($id)) {
            $valorDesconto = $valorDescontoBD->descontoservico + $valorDescontoBD->descontoproduto;
            $totalSemDesconto = $totalProdutos +  $totalServico;
        ...
        return ['totalServico' => $totalServico, 'totalProdutos' => $totalProdutos, 'valor_desconto' => $valorDesconto,
         'totalSemDesconto' => $totalSemDesconto, 'totalComDesconto' => $totalComDesconto];
    }
    ...
    public function getQrCode($id, $pixKey, $emitente)
    {
        ...
        $amount = $result['valor_desconto'] != 0 ? round(floatval($result['totalServico'] + $result['totalProdutos'] - $result['valor_desconto']), 2) : round(floatval($result['totalServico'] + $result['totalProdutos']), 2);
