...
class Mapos_model extends CI_Model
{
    ...
    public function pesquisar($termo)
    {
        ...
        if(count($data['clientes']) >= 1){
            $this->db->or_like('clientes_id', $data['clientes'][0]->id_clientes);
        }
    ...
    public function getOsIndex($aux)
    {
        $this->db->select('os.*, clientes.nomeCliente');
        $this->db->from('os');
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
        $this->db->where('os.status', $aux);
        $this->db->limit(10);
    ...
    {
        return $this->db->get('logos')->result();
    }

    public function editLogos($id, $logo)
    {
        $this->db->set('logo', $logo);
        $this->db->where('id', $id);
        return $this->db->update('logos');
    }

    public function editLogosIcon($id, $logo)
    {
        $this->db->set('favicon', $logo);
        $this->db->where('id', $id);
        return $this->db->update('logos');
    }
