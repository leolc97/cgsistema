...
class Vendas_model extends CI_Model
{
    ...
    public function getQrCode($id, $pixKey, $emitente)
    {
        ...
        $amount = $valorDesconto->descontovenda != 0 ? round(floatval($totalProdutos - $valorDesconto->descontovenda), 2) : round(floatval($totalProdutos), 2);
    ...
    public function getAnexos($vendas)
    {
        $this->db->where('vendas_id', $vendas);
        return $this->db->get('anexos_vendas')->result();
    }

    public function anexar($vendas, $anexo, $url, $thumb, $path)
    {
        $this->db->set('anexo', $anexo);
        $this->db->set('url', $url);
        $this->db->set('thumb', $thumb);
        $this->db->set('path', $path);
        $this->db->set('vendas_id', $vendas);

        return $this->db->insert('anexos_vendas');
    }
}
